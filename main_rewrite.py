...
def key_down_callback(key):
    print(key)
    print(normalize(str(key)))
    # db.insert(table="keys", key=key, down=1, time=time.time())


def key_up_callback(key):
    key = str(key)
    print(key)
    db.insert(table="keys", key=key, down=0, time=time.time())


def mouse_click_callback(x, y, button, pressed):
    button = str(button)[7::]
    print(button)
    db.insert(table="mouse_buttons", button=button, pressed=int(pressed), time=time.time())
...
def mouse_button_parse() -> dict:
    ...
    # used for filtering row by buttond
    uniq_buttons = []
...
keyboard_hook = keyboard.Listener(
    on_press=lambda x: safe_call(lambda: key_down_callback(x)),
    on_release=lambda x: safe_call(lambda: key_up_callback(x))
)

mouse_hook = mouse.Listener(
    on_click=lambda x, y, button, pressed: safe_call(lambda: mouse_click_callback(x, y, button, pressed))
)
keyboard_hook.start()
mouse_hook.start()
